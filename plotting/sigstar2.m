...
%          strings corresponding to X-ticklabels. Groups can be a mixture of both
%          definition types.
...
% check if we need to rescale the y axis
if thisY < currentYlims(1),
    ylim([currentYlims(1)-(0.5*abs(currentYlims(1))) currentYlims(2)]);
elseif thisY > currentYlims(2),
    ylim([currentYlims(1) currentYlims(2)+(0.5*abs(currentYlims(2)))]);
end
...
% only plot a line if connecting two groups
if length(unique(x)) > 1,
    H(1)=plot(x(:),y,'-k','LineWidth',0.5);
end
...
if p <=0.05,
    H(2)=text(mean(x(:)),mean(y)+myRange(ylim)*offset,stars,...
        ...
        'BackGroundColor','none', 'fontsize', 7);
...
%Find the corresponding ydy values
clear xd
yd=get(p,'YData');
if iscell(yd)
...
% AEU edit: also get upper data of the error bar
ud=get(p,'UData');
if iscell(ud)
    ud=groupedBarFix(ud,'y');
end
ud=ud(:,overlapping);

% add the two together so the sigstar bar sits above the upper errorbar.
Y=max(yd(:) + ud(:));
